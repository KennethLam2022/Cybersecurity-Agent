...
def test_mcp_policy_defaults_to_deny_and_audits_redacted_parameters(tmp_path):
    ...
    owner = memory.register_user("mcp-policy@example.com", "Correct-Horse-30", "MCP Policy")
    tenant_id, agent_id = owner["tenant_id"], owner["agent_id"]
    store = GovernanceStore(str(db_path))
    secret = store.create_secret(tenant_id, "mcp-token", "mcp", "token", "admin", "org_admin")
    server = store.register_mcp_server(tenant_id, "intel", "https://mcp.example.test", "admin", secret_ref_id=secret["id"], auth_type="bearer")
    store.upsert_mcp_tool(tenant_id, server["id"], "search", "lookup", {"type": "object"}, False, "admin", "org_admin")
    assert not store.authorize_mcp_call(tenant_id, server["id"], "search", user_id="u", role="org_admin", agent_id=agent_id, params={"query": "x"})
    store.set_mcp_status(tenant_id, server["id"], "enabled", "admin", "org_admin")
    store.set_mcp_tool_policy(tenant_id, server["id"], "search", allowed_roles=["org_admin"], allowed_agents=[agent_id], param_allowlist=["query"], param_denylist=["token"], enabled=True, actor_user_id="admin", actor_role="org_admin")
    assert store.authorize_mcp_call(tenant_id, server["id"], "search", user_id="u", role="org_admin", agent_id=agent_id, params={"query": "x"})
    assert not store.authorize_mcp_call(tenant_id, server["id"], "search", user_id="u", role="org_admin", agent_id=agent_id, params={"token": "secret"})
    store.record_mcp_call(tenant_id, server["id"], "search", "u", agent_id, {"query": "x", "token": "secret"}, "ok", 12)
    with store._connect() as conn:
        summary = conn.execute("SELECT param_summary FROM p8_mcp_call_audit").fetchone()[0]
    assert "secret" not in summary


def test_mcp_execution_requires_an_active_agent_in_the_same_tenant(tmp_path):
    db_path = tmp_path / "mcp-agent-scope.db"
    memory = ConversationMemory(str(db_path))
    owner_a = memory.register_user("mcp-a@example.com", "Correct-Horse-30", "Tenant A")
    owner_b = memory.register_user("mcp-b@example.com", "Correct-Horse-30", "Tenant B")
    store = GovernanceStore(str(db_path))
    tenant_a = owner_a["tenant_id"]
    secret = store.create_secret(tenant_a, "mcp-token", "mcp", "token", "admin", "org_admin")
    server = store.register_mcp_server(tenant_a, "intel", "https://mcp.example.test", "admin", secret_ref_id=secret["id"], auth_type="bearer")
    store.upsert_mcp_tool(tenant_a, server["id"], "search", "lookup", {"type": "object"}, False, "admin", "org_admin")
    store.set_mcp_status(tenant_a, server["id"], "enabled", "admin", "org_admin")
    store.set_mcp_tool_policy(tenant_a, server["id"], "search", allowed_roles=["org_admin"], allowed_agents=[], param_allowlist=[], param_denylist=[], enabled=True, actor_user_id="admin", actor_role="org_admin")

    assert store.authorize_mcp_call(owner_a["tenant_id"], server["id"], "search", user_id="u", role="org_admin", agent_id=owner_a["agent_id"], params={})
    assert not store.authorize_mcp_call(owner_a["tenant_id"], server["id"], "search", user_id="u", role="org_admin", agent_id=owner_b["agent_id"], params={})
    assert not store.authorize_mcp_call(tenant_a, server["id"], "search", user_id="u", role="org_admin", agent_id="missing", params={})
...
def test_operations_task_supports_assignment_sla_comments_and_tenant_scope(tmp_path):
    db_path = tmp_path / "governance.db"
    memory = ConversationMemory(str(db_path))
    owner = memory.register_user("task-owner@example.com", "Correct-Horse-30", "Task Workspace")
    reviewer = memory.create_workspace_user(owner["tenant_id"], "task-reviewer@example.com", "Correct-Horse-31", "Reviewer")["id"]
    store = GovernanceStore(str(db_path))
    task = store.create_operations_task(owner["tenant_id"], owner["id"], source_type="knowledge_gap", source_id="7", title="补充资料", priority="high", assignee_user_id=owner["id"], collaborator_user_ids=[reviewer], sla_hours=24)
    assert store.list_operations_tasks("tenant-b") == []
    updated = store.update_operations_task(owner["tenant_id"], task["id"], owner["id"], {"status": "in_progress", "department": "research"})
    assert updated["department"] == "research"
    comment = store.add_operations_task_comment(owner["tenant_id"], task["id"], reviewer, "已开始核查")
    assert comment["task_id"] == task["id"]
    assert store.update_operations_task("tenant-b", task["id"], "admin-b", {"status": "done"}) is None


def test_operations_tasks_and_notification_policies_reject_cross_tenant_users(tmp_path):
    db_path = tmp_path / "governance-user-scope.db"
    memory = ConversationMemory(str(db_path))
    owner_a = memory.register_user("scope-a@example.com", "Correct-Horse-30", "Scope A")
    owner_b = memory.register_user("scope-b@example.com", "Correct-Horse-30", "Scope B")
    store = GovernanceStore(str(db_path))

    with pytest.raises(ValueError, match="不属于当前工作区"):
        store.create_operations_task(
            owner_a["tenant_id"], owner_a["id"], source_type="manual",
            title="跨组织任务", assignee_user_id=owner_b["id"],
        )
    with pytest.raises(ValueError, match="不属于当前工作区"):
        store.upsert_notification_policy(
            owner_a["tenant_id"], owner_a["id"], {
                "name": "跨组织通知", "event_types": ["eval.completed"],
                "recipient_user_ids": [owner_b["id"]],
            },
        )
...
def test_notification_policy_controls_channels_and_quiet_window(tmp_path):
    db_path = tmp_path / "governance.db"
    memory = ConversationMemory(str(db_path))
    owner = memory.register_user("notification-owner@example.com", "Correct-Horse-30", "Notification Workspace")
    reviewer = memory.create_workspace_user(owner["tenant_id"], "notification-reviewer@example.com", "Correct-Horse-31", "Reviewer")["id"]
    store = GovernanceStore(str(db_path))
    policy = store.upsert_notification_policy(owner["tenant_id"], owner["id"], {"name": "eval", "event_types": ["eval.completed"], "channels": ["in_app"], "recipient_user_ids": [reviewer], "cooldown_minutes": 10})
    decision = store.notification_decision(owner["tenant_id"], "eval.completed", "info")
    assert decision["channels"] == ["in_app"] and decision["policy_ids"] == [policy["id"]]
    assert decision["recipient_user_ids"] == [reviewer]
    assert store.notification_decision(owner["tenant_id"], "eval.completed", "info")["suppressed"] is True
    assert store.notification_decision(owner["tenant_id"], "quota.warning", "warning")["channels"] == ["in_app", "email", "webhook"]
    assert any(item["decision"] == "suppressed" for item in store.list_notification_policy_deliveries(owner["tenant_id"], policy["id"]))
