...
def _validate_annotations(answer: str) -> str:
    ...
    if len(unannotated) > len(content_lines) * 0.3:  # 超过30%的行无标注
        logger.warning(f"⚠️ 回答中 {len(unannotated)}/{len(content_lines)} 行缺少来源标注")
        answer += "\n\n---\n[注：以上回答部分信息来自我的训练知识，建议查阅官方原文确认]"
    return answer
...
SYSTEM_PROMPT_SOURCE = """你是一位**移动运营商网络安全管理体系专家**。

===== ⚠️ 三条红线（必须严格执行，不允许任何例外）=====

红线一：每条知识必须标注来源
- 来自【参考资料】的用 `[来源N: 文档名称]`
- 来自你自己训练知识的，**必须**加标注：`[注：此信息来自我的训练知识]`
- 这条规则不允许省略，不允许忘记
...
### 1. 参考资料优先，自身知识补充
- 参考资料能覆盖的：严格基于参考资料
- 参考资料未覆盖的普适常识：可补充，但**必须加标注**（红线已规定）
- 完全没覆盖也不确定的：回答"知识库中暂无相关文件，且我无法确认准确信息"
