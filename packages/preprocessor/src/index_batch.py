...
def run_batch(batch_num):
    ...
    logger.info(
        f"  Chroma 全量重建：{len(all_for_chroma)} 份文件（已有 {len(all_indexed)} + 新增 {len(batch_files)}）")
...
def show_status():
    ...
    logger.info(f"  向量化进度")
    logger.info(f"{'='*50}")
    logger.info(f"  总文档:    {len(all_files)} 份")
    logger.info(f"  已索引:    {indexed} 份")
    logger.info(f"  未索引:    {len(all_files) - indexed} 份")
    logger.info(f"  总chunks:  {progress['stats']['total_chunks']}")
    logger.info(f"  已完成批次:  {progress['batches_done']}")
    remaining = len(all_files) - indexed
    if remaining > 0:
        next_batch = max(progress["batches_done"]) + 1 if progress["batches_done"] else 1
        logger.info(f"  下一批次:  --batch {next_batch}")
    # Chroma 状态
    chroma_dir = os.path.join(str(STORE_DIR), "chroma_db")
    if os.path.exists(chroma_dir):
        try:
            ...
            logger.info(f"  Chroma: {col.count()} chunks")
        except Exception as e:
            logger.info(f"  Chroma: 读取失败 ({e})")
    # FAISS 状态
    if os.path.exists(os.path.join(_FAISS_DIR, "index.faiss")):
        logger.info(f"  FAISS: 存在 ({_FAISS_DIR})")
...
if __name__ == "__main__":
    ...
    if args.reset:
        ...
        save_progress({"indexed": [], "batches_done": [], "stats": {
